...
class Brain_Age_Splitting_Wrapper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        BenisLänge = X.shape[1] - self.patch_size
        BenisSchritte = BenisLänge / self.patch_size
        KleineBenisSchritte = math.trunc(BenisSchritte)
        BenisSteppos = KleineBenisSchritte * KleineBenisSchritte
        ...
        for i in range(X.shape[0]):
            ...
            BenisLänge = X.shape[0]
            #print(BenisLänge)
            BenisSchritte = BenisLänge / self.patch_size
            BenisMatrix = Benis[1:BenisLänge:self.patch_size, 1:BenisLänge:self.patch_size, :, :]
