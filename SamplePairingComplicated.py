...
from photonai.modelwrapper.SimpleCNN import SimpleCNN
from photonai.modelwrapper.Biclustering2d import Biclustering2d
...
def get_samples(X, y=None, generator='random_pair', distance_metric=None, draw_limit=10000, rand_seed=True):
    ...
    X_aug = np.empty([len(combs), X.shape[1], X.shape[2]])
    ...
    for c in combs:
        ...
        X_aug[i] = np.squeeze(np.mean([X[c[0]], X[c[1]]], axis=0))
...
def run_classification(X_tr, y_tr):
    ...
    #estimators.append(('bicluster', Biclustering2d()))

    estimators.append(('ConvNet', SimpleCNN()))
...
if __name__ == '__main__':
    ...
    for dl in drawLimitList:
        ...
        mat_data = sio.loadmat('/home/vholstein/tmp/pycharm_project_171/Dosenbach_matrices_1044.mat')
        FC_matrices = mat_data['matrices']
        FC_reordered = np.swapaxes(FC_matrices, 0, 2)

        FC_labels = np.loadtxt('/home/vholstein/tmp/pycharm_project_171/Dosenbach_labels_1044.txt')
        ...
        X_tr, y_tr = get_samples_classification(X_tr, y_tr, generator='random_pair', distance_metric=None,
        ...
        my_model = run_classification(X_tr=X_tr, y_tr=y_tr)
        ...
        results = accuracy_score(y_te, preds)

        #results = my_model.score(X_te, y_te)

        print("ACC Test %.2f" % results)
